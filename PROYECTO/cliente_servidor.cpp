...
std::string calculate_sha256(const char* data, size_t len) {
...
void xor_encrypt_decrypt(char* data, size_t len) {
    const std::string key = "ClaveSecreta";
...
int main() {
    ...
    while (connect(clientSocket, (sockaddr*)&serverAddr, sizeof(serverAddr)) < 0) {
        std::cerr << "No se pudo conectar al servidor. Reintentando en 5 segundos..." << std::endl;
        sleep(5);
    }
    ...
    while (true) {
        ...
        std::string hash = calculate_sha256((char*)&packet, data_to_sign_len);
